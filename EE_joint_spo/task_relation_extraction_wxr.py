...
model_root = "/media/mawei/123bd683-36a2-4819-b886-69a51796e8501/chenjun/models/chinese_L-12_H-768_A-12"
config_path = os.path.join(model_root,'bert_config.json')
checkpoint_path = os.path.join(model_root,'bert_model.ckpt')
dict_path = os.path.join(model_root,'vocab.txt')
...
# 加载数据
# data_root = "/media/mawei/123bd683-36a2-4819-b886-69a51796e8501/chenjun/data/knowledege-extract"
data_root = "/media/mawei/123bd683-36a2-4819-b886-69a51796e8501/chenjun/ccks_program/doter/sujianlin/lic2020_baselines-master/data"
train_data = load_data(os.path.join(data_root,'train_data.json'))
valid_data = load_data(os.path.join(data_root,'dev_data.json'))
predicate2id, id2predicate = {}, {}

with open(os.path.join(data_root,'schema.json'),encoding="utf-8") as f:
    for l in f:
        l = json.loads(l)
...
def extract_spoes(text):
    ...
    # print("subject_preds:",subject_preds)
...
def inference(test_file):
    result = []
    test_d = pd.read_csv(test_file, sep='\t', header=None, encoding='utf-8')
    for _, row in test_d.iterrows():
        id, content = row[0], row[1]
        spos = extract_spoes(content)
        if not spos:
            continue
        for spo in spos:
            event_subject, event_class = spo[0], spo[1]
            result.append(
                {
                    "index": id,
    ...
    result_df = pd.DataFrame(result, columns=['index', 'event_class', 'event_subject'])
    result_df.drop_duplicates(subset=['index', 'event_class', 'event_subject'])
    result_df.to_csv('./result_spo_6_28.csv', header=None, index=0, sep='\t')
...
if __name__ == '__main__':
    ...
    res = extract_spoes('腾讯证券5月23日讯，*ST康得收到关注函：公司控股股东康得投资集团有限公司信托贷款出现违约，康得新光电作为担保方承担连带担保责任')
    print(res)
    test_file_path = os.path.join(data_root, 'event_entity_dev_data.csv')
    inference(test_file_path)